...
public static class AcsClipBuilder
{
    extension(Card owner)
    {
        ...
        /// </summary>
        public AcsClip CreateAcsClip(Sprite[] sprites,
                                     string clipName = "",
                                     float interval = -1f,
                                     AcsAnimationType type = AcsAnimationType.Auto)
        {
            ...
            AcsMod.Log(
                $"created clip {owner.id}/{name}/{sprite.rect.width}x{sprite.rect.height}/{sprites.Length} frames@{interval}");
            return new() {
                name = name,
                type = type,
                owner = owner.id,
        ...
        /// </summary>
        public List<AcsClip> CreateAcsClips(IEnumerable<Sprite> sprites)
        {
