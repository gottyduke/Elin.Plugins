...
internal partial class CwlConsole
{
    ...
    {
        const string header = "id,name,STR,END,DEX,PER,LER,WIL,MAG,CHA,STR POT,END POT,DEX POT,LER POT";
        ...
        foreach (var r in EClass.sources.races.rows.Where(checkedRaces.Add)) {
            ...
            foreach (var exporter in exporters) {
                ...
                exporter.sb.AppendLine($"{r.id},{r.GetName()}," +
                                       ...
                                       $"{Value(food.Evalue(ENC.sustain_STR))},{Value(food.Evalue(ENC.sustain_END))}," +
                                       $"{Value(food.Evalue(ENC.sustain_DEX))},{Value(food.Evalue(ENC.sustain_LER))}");
            }
            continue;
