...
[HarmonyPatch]
internal class LaserByTraitPatch : EClass
{
    [HarmonyPrefix]
    ...
    internal static bool OnPlayDelayedEffect(AttackProcess __instance, int numFire)
    {
        ...
        var id = weapon.id;
        var dataEx = DataLoader.CachedEffectData.GetValueOrDefault(__instance.weapon.id);
        ...
        var fireOffset = cc.IsPCC
            ? data.firePos with { x = data.firePos.x * (cc.renderer.actor.currentDir is not (0 or 1) ? 1 : -1) }
            : Vector2.zero;
