...
public static class PartExt
{
    ...
    public static int PartSorter(PCC.Part lhs, PCC.Part rhs)
    {
        ...
        while (i < lhsMatches.Count && j < rhsMatches.Count) {
            ...
            var lhsPartIntegral = int.TryParse(lhsPart, out var lhsPartNum);
            var rhsPartIntegral = int.TryParse(rhsPart, out var rhsPartNum);

            if (lhsPartIntegral && rhsPartIntegral) {
                var cmp = lhsPartNum.CompareTo(rhsPartNum);
                if (cmp != 0) {
                    return cmp;
                }
            } else {
