...
internal static class CharaBuildCompleteEvent
{
    ...
    [HarmonyPrefix, HarmonyPatch(typeof(TaskBuild), nameof(TaskBuild.OnProgressComplete))]
    ...
    {
        if (thiz.useHeld) {
            if (thiz.owner.held == null || thiz.owner.held.GetRootCard() != thiz.owner || thiz.pos.Distance(thiz.owner.pos) > 1 || !thiz.pos.IsInBounds) {
                return;
            }
            ...
            if (thiz.target.id == "statue_weird") {
                thiz.owner.Say("statue_install", null, null);
            }
        }
        ...
        if (ActionMode.Build.IsActive && ActionMode.Build.IsFillMode()) {
            if (thiz.recipe.IsBridge) {
                ...
                thiz.bridgeHeight = (int)thiz.pos.cell.bridgeHeight;
                thiz.altitude = 0;
            ...
            Effect.Get("smoke").Play(thiz.pos, 0f, null, null);
            Effect.Get("mine").Play(thiz.pos, 0f, null, null).SetParticleColor(thiz.recipe.GetColorMaterial().GetColor())
            ...
            if (thiz.recipe.IsWallOrFence) {
                if (thiz.pos.HasWallOrFence && thiz.pos.cell.blockDir != 2 && thiz.pos.cell.blockDir != thiz.recipe._dir) {
                    ...
                    thiz.owner.PlaySound(thiz.pos.matBlock.GetSoundImpact(null), 1f, true);
                    thiz.pos.RefreshTile();
                    return;
                }
                if (thiz.pos.sourceRoofBlock.tileType.IsWallOrFence && thiz.pos.cell._roofBlockDir % 4 != 2 && (int)(thiz.pos.cell._roofBlockDir % 4) != thiz.recipe._dir) {
        ...
        EClass._map.RefreshFOV(thiz.pos.x, thiz.pos.z, 6, false);
        thiz.owner.renderer.SetFirst(true);
        if (thiz.recipe.IsFloor) {
            foreach (Card card in thiz.pos.ListThings<TraitNewZone>(true)) {
                bool isDownstairs = (card.trait as TraitNewZone)!.IsDownstairs;
            }
        }
        if (!ActionMode.Build.IsActive || !ActionMode.Build.IsRoofEditMode(null)) {
            thiz.pos.ForeachMultiSize(thiz.recipe.W, thiz.recipe.H, delegate (Point p, bool center) {
                if (p.IsBlocked && p.HasChara) {
                    foreach (Chara chara in p.ListCharas()) {
                        thiz.owner.Kick(chara, true, false, false, true);
                    }
                }
            });
        }
