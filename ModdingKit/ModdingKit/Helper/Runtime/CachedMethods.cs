    ...
    extension(MethodInfo method)
    {
        ...
        public bool ValidateParameterTypes(bool warn, IReadOnlyList<(string? type, string? name)> types)
        {
            ...
            if (parameters.Length != types.Count) {
                if (warn) {
                    Debug.Log($"{method.Name} parameter count mismatch: expected {parameters.Length}, got {types.Count}");
                }
            ...
            for (var i = 0; i < parameters.Length; ++i) {
                ...
                if (string.IsNullOrEmpty(type)) {
                    ...
                    Debug.Log($"{method.Name} parameter {i} type mismatch: expected {paramType.Name}, got null !!(value type)");
                    return false;
                }
                ...
                Debug.Log($"{method.Name} parameter {i} type mismatch: expected {paramInfo}, got {type}:{name}");
                return false;
            }
